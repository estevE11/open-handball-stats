...
// Keep only XML 1.0 characters, including valid supplementary Unicode.
    ...
    [
      ...
      ...match.events.map((e) =>
        [
          ...
          e.period,
          e.gameTimeSeconds,
          new Date(e.realTimestamp).toISOString(),
          e.attackingTeamName,
          e.defendingTeamName,
          e.attackPhase,
          e.defenseSystem,
...
export function exportJSON(match: MatchSession): string {
  return JSON.stringify(stopClock(match), null, 2);
}
...
export function exportXML(match: MatchSession, offsetSeconds = 0): string {
  ...
  const codes = [
    ...new Set(
      match.events.map((e) => `${e.attackingTeamName} · ${e.eventType}`),
    ),
  ];
  const instances = match.events.map((e, i) => {
    const time = Math.max(0, e.gameTimeSeconds + offsetSeconds);
    const labels = {
      possession_index: e.possessionIndex,
      period: e.period,
      attacking_team: e.attackingTeamName,
      defending_team: e.defendingTeamName,
      attack_phase: e.attackPhase,
      defense_system: e.defenseSystem,
      event_type: e.eventType,
      sub_type: e.subType ?? "",
      is_possession_flipped: e.isPossessionFlipped,
      timestamp: new Date(e.realTimestamp).toISOString(),
      sanction_team: e.sanctionTeamId ?? "",
      notes: e.notes ?? "",
      event_id: e.id,
      match_id: e.matchId,
    };
    return `    <instance><ID>${i + 1}</ID><start>${Math.max(0, time - 5).toFixed(3)}</start><end>${(time + 3).toFixed(3)}</end><code>${xml(`${e.attackingTeamName} · ${e.eventType}`)}</code>${Object.entries(
      labels,
    )
      .map(
        ([group, text]) =>
          `<label><group>${group}</group><text>${xml(text)}</text></label>`,
      )
      .join("")}</instance>`;
  });
...
export function importJSON(source: string): MatchSession {
  if (new TextEncoder().encode(source).length > 20 * 1024 * 1024)
...
export function downloadMatch(
  ...
  const contents = {
    csv: () => exportCSV(match),
    json: () => exportJSON(match),
    xml: () => exportXML(match, offset),
  }[format]();
  const mime = {
