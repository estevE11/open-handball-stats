...
export const eventSchema = z.object({
  id: text,
  matchId: text,
  possessionIndex: z.number().int().positive(),
  period: z.number().int().min(1).max(20),
  gameTimeSeconds: seconds,
  realTimestamp: z.number().int().min(0).max(8640000000000000),
  attackingTeamName: text,
  defendingTeamName: text,
  attackingTeamId: z.enum(["home", "away"]).optional(),
  attackPhase: z.enum(attackPhases),
  defenseSystem: z.enum(defenseSystems),
  eventType: z.enum(eventTypes),
  subType: z.enum([...technicalFaults, ...sanctions]).optional(),
  sanctionTeamId: z.enum(["home", "away"]).optional(),
  isPossessionFlipped: z.boolean(),
  notes: z.string().max(2000).optional(),
});
const teamSchema = z.object({
  name: text,
  color: z.string().regex(/^#[0-9a-f]{6}$/i),
  currentDefense: z.enum(defenseSystems),
});
export const matchSchema = z
  .object({
    ...
    homeTeam: teamSchema,
    awayTeam: teamSchema,
    currentAttackingTeamId: z.enum(["home", "away"]),
    currentPossessionIndex: z.number().int().positive(),
  ...
  .superRefine((match, ctx) => {
    ...
    if (match.homeTeam.name === match.awayTeam.name)
      bad("Team names must differ.");
    if (
      new Set(match.events.map((event) => event.id)).size !==
      match.events.length
    )
      bad("Duplicate event identifiers.");
    for (const event of match.events) {
      if (event.matchId !== match.id) bad("Event belongs to another match.");
      if (
        event.possessionIndex > match.currentPossessionIndex ||
        event.period > match.period
      )
        bad("Event exceeds current match state.");
      const names = [match.homeTeam.name, match.awayTeam.name];
      if (
        !names.includes(event.attackingTeamName) ||
        !names.includes(event.defendingTeamName) ||
        event.attackingTeamName === event.defendingTeamName
      )
        bad("Invalid event teams.");
      if (
        event.attackingTeamId &&
        match[event.attackingTeamId === "home" ? "homeTeam" : "awayTeam"]
          .name !== event.attackingTeamName
      )
        bad("Inconsistent team identifier.");
      const flips = [
        "GOAL",
        "GK_SAVE",
        "SHOT_OUT",
      ...
      if (
        ...
        !technicalFaults.includes(event.subType as TechnicalFaultType)
      )
        bad("Missing technical fault subtype.");
      if (
        event.eventType === "SANCTION" &&
        (!sanctions.includes(event.subType as SanctionType) ||
          !event.sanctionTeamId)
      )
        bad("Missing sanction details.");
      if (
        !["TECHNICAL_FAULT", "SANCTION"].includes(event.eventType) &&
        event.subType
      )
        bad("Unexpected subtype.");
    }
  });
